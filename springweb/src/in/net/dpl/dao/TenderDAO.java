...
public class TenderDAO {
	...
	public int saveTender(TenderModel t,String fileName){  
	    String query="insert into TenderMasterNew(tender_group,tender_ref_no,tender_type,estimated_value,opening_tender_sale,closing_tender_sale,date_pre_bid,due_date_bid_sub,scope_of_work,tender_upload_date,file_name) "
	    		+ "values('"+t.getTenderGroup()+"','"+t.getTenderRefNo()+"','"+t.getTenderType()+"','"+t.getEstimatedValue()+"',str_to_date('"+t.getOpeningDateTime()+"','%d-%m-%Y %H:%i'),str_to_date('"+t.getClosingDateTime()+"','%d-%m-%Y %H:%i'),str_to_date('"+t.getPrebidDateTime()+"','%d-%m-%Y %H:%i'),str_to_date('"+t.getSubmissionDateTime()+"','%d-%m-%Y %H:%i'),'"+t.getScope()+"',NOW(),'"+fileName+"')";  
